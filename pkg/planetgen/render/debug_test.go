...
func TestDebugFrameNoBandsWhenSingleInterval(t *testing.T) {
	...
	frame := &DebugFrame{}
	_, _, _ = generateRockyHeightmapDebug(&prof, 42, 16, frame, nil, nil, nil)
...
func TestDebugFrameStageOrder(t *testing.T) {
	...
	frame := &DebugFrame{}
	_, _, _ = generateRockyHeightmapDebug(&prof, 42, 32, frame, nil, nil, nil)
	if len(frame.Stages) == 0 {
...
func TestDebugFrameBypassSkipFlag(t *testing.T) {
	...
	_, _, _ = generateRockyHeightmapDebug(&prof, 42, 16, frame, bypass, nil, nil)
	if len(frame.Stages) == 0 {
		t.Fatal("no stages recorded")
	}
	s := frame.Stages[0]
...
func TestDebugFrameBypassParity(t *testing.T) {
	prof := minimalRockyProfile()
	bypass := DebugBypass{"Continentalness": true}
	hm, _, _ := generateRockyHeightmapDebug(&prof, 42, 16, nil, bypass, nil, nil)
...
func TestDebugFrameNilFrameNoPanic(t *testing.T) {
	prof := minimalRockyProfile()
	hm, _, _ := generateRockyHeightmapDebug(&prof, 42, 16, nil, nil, nil, nil)
	if hm == nil {
...
func TestDebugFrameColorStages(t *testing.T) {
	...
	hm, craters, _ := generateRockyHeightmapDebug(&prof, 42, 16, nil, nil, nil, nil)
	frame := &DebugFrame{}
